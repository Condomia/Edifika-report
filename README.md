...
TB4: Actualicé conocimientos en DDD y Spring Security para codificar el microservicio de foros comunitarios, aplicando filtros JWT personalizados y gestionando el ciclo de vida del Sprint mediante el Kanban Board del equipo (secciones 5.3.3.7, 5.3.3.8).<br><br>
...
TB4: Actualicé conocimientos de desarrollo frontend para mejorar el dashboard y las secciones de inicio, perfil y configuración, optimizando la experiencia de usuario del sistema (sección 5.3.3.1).<br><br>
TF: Actualicé y apliqué conocimientos de desarrollo frontend para implementar los módulos Finance & Reports y Community Wall de la aplicación web Edifika. En el módulo financiero desarrollé la visualización de indicadores como ingresos totales, deudas pendientes, morosidad, tasa de recaudación, tasa de ocupación, ingresos mensuales y residentes con saldos pendientes, utilizando tarjetas informativas, gráficos, barras de progreso y tablas. Asimismo, en el Community Wall implementé la interfaz para crear y visualizar publicaciones de residentes, incluyendo contenido textual, información del autor, imágenes adjuntas y accesos a las normas de convivencia. También participé en la elaboración del Sprint Backlog 4, organizando las historias de usuario y tareas del Sprint, y en la Development Evidence for Sprint Review, documentando los avances y commits relacionados con la implementación final del frontend (secciones 5.3.4.1, 5.3.4.2).<br><br>
...
TF: Implementé la arquitectura basada en microservicios para la plataforma Edifika utilizando Spring Boot, DDD y Clean Architecture. Desarrollé servicios independientes para los módulos principales, implementé comunicación mediante APIs REST y documenté cada microservicio con Swagger, fortaleciendo mis conocimientos en diseño de arquitecturas escalables y mantenibles.
</td>
<td>
...
TF: Reconocí que implementar interfaces financieras y sociales conectadas con diferentes servicios backend exige aprendizaje permanente en consumo de APIs REST, procesamiento de información, representación de indicadores y manejo de contenido multimedia. El desarrollo de Finance & Reports me permitió profundizar en la transformación de datos financieros en gráficos, métricas y tablas comprensibles para los administradores, mientras que la implementación del Community Wall reforzó mis conocimientos sobre formularios, publicación de contenido, carga de imágenes y diseño de experiencias de interacción comunitaria. Asimismo, la elaboración del Sprint Backlog 4 y de la Development Evidence for Sprint Review me permitió comprender la importancia de mantener una adecuada planificación, trazabilidad y documentación de los cambios realizados durante el desarrollo de una solución de software (secciones 5.3.4.1, 5.3.4.2).<br><br>
Ortiz Cardenas, Johanna Antuanete<br><br>
...
En esta sección se aplicó la metodología Behavior-Driven Development (BDD) para definir y verificar el comportamiento esperado del microservicio IAM desde la perspectiva técnica y funcional. Se elaboraron archivos de características (.feature) utilizando la sintaxis Gherkin de Cucumber, donde cada escenario describe una interacción específica del sistema mediante la estructura Given–When–Then.
...
Esta sección resume la evidencia de ejecución del Sprint 1, donde se verificó el correcto funcionamiento de los endpoints del microservicio IAM de Edifika. Las pruebas se realizaron mediante Swagger UI, validando los flujos de autenticación, gestión de usuarios y asignación de roles. Se comprobó que el sistema genera tokens JWT al iniciar sesión y que los endpoints protegidos responden correctamente cuando se proporciona un Bearer Token válido. A continuación se detallan las pruebas realizadas por cada endpoint:
...
Se verificó la consulta de un usuario específico por su id, enviando el Bearer Token en el header Authorization. El sistema retornó un código 200 con los datos completos del usuario incluyendo fullName, email, phone, status, documentType, documentNumber y roles asignados.
...
Se verificó la eliminación de un usuario del sistema enviando el id como parámetro con el Bearer Token en el header. El sistema procesó la solicitud y confirmó la eliminación del registro.
...
Durante este sprint, el equipo concentró sus esfuerzos en completar y consolidar las funcionalidades principales de Edifika, fortaleciendo los módulos de gestión de usuarios, administración de edificios y unidades, reservas de áreas comunes, pagos, comunicados, morosidad y consulta de documentación interna. A diferencia del Sprint 3, donde se inició el desarrollo de diversos flujos funcionales y microservicios especializados, en el Sprint 4 se priorizó la finalización de las historias de usuario pendientes y la integración de nuevas capacidades orientadas a la administración integral del condominio.

El objetivo principal del Sprint 4 fue completar funcionalidades clave como la actualización y verificación de información de usuarios, el registro de edificios y unidades, la aprobación, rechazo y cancelación de reservas, la prevención de reservas duplicadas, la configuración y disponibilidad de áreas comunes, el registro de pagos, la visualización de residentes morosos y la generación de reportes financieros. Asimismo, se completó la configuración base de los microservicios Notification Service, integrado con Firebase Cloud Messaging para el envío de notificaciones push, y Report Service, orientado a la generación y exportación de reportes financieros y de actividad del condominio. Con ello, se logró ampliar la capacidad operativa, administrativa y comunicacional de la plataforma Edifika.

| Código US / TS | Título | Id de Tarea | Título de la Tarea | Descripción | Estimación (hrs) | Asignado a | Estado |
|---|---|---:|---|---|---:|---|---|
| US05 | Actualizar información de usuarios | T43 | Implementar actualización de usuarios | Implementar la funcionalidad para que el administrador pueda modificar los datos personales y de contacto de los usuarios registrados, validando la información antes de guardar los cambios. | 4 | Ortiz Cardenas, Johanna Antuanete | Done |
| US18 | Aprobar o rechazar reservas | T44 | Implementar aprobación y rechazo de reservas | Desarrollar la funcionalidad administrativa para revisar solicitudes de reserva y cambiar su estado a aprobada o rechazada, registrando la decisión tomada. | 5 | Zegarra Lopez, Renato Sebastian Rubber | Done |
| US19 | Evitar reservas duplicadas | T45 | Validar duplicidad de reservas | Implementar una validación que impida registrar dos reservas para la misma área común, fecha y horario, evitando conflictos de disponibilidad. | 4 | Bejarano Martinez, Alvaro Leandro | Done |
| US25 | Generar reportes financieros | T46 | Implementar generación de reportes financieros | Desarrollar la consulta consolidada de pagos, deudas, morosidad e ingresos generados por el condominio para presentar indicadores financieros al administrador. | 6 | Sarmiento Medina, Loreley | Done |
| US33 | Ver disponibilidad global (Admin) | T47 | Implementar vista global de disponibilidad | Crear una vista administrativa que permita consultar la disponibilidad de todas las áreas comunes por fecha, horario, estado y reservas registradas. | 5 | Becerra Tejeda, Alessandra Nicole | Done |
| US35 | Cancelar reserva (Admin) | T48 | Implementar cancelación administrativa de reservas | Permitir que el administrador cancele una reserva activa, actualizando su estado, liberando el horario reservado y registrando el motivo de cancelación. | 4 | Ortiz Cardenas, Johanna Antuanete | Done |
| US38 | Habilitar o deshabilitar área común | T49 | Implementar cambio de estado de áreas comunes | Desarrollar la funcionalidad para habilitar o deshabilitar áreas comunes cuando se encuentren disponibles, en mantenimiento o temporalmente fuera de servicio. | 4 | Zegarra Lopez, Renato Sebastian Rubber | Done |
| US39 | Configurar reglas de área común | T50 | Implementar configuración de reglas | Permitir que el administrador configure el tiempo máximo de reserva, precio, garantía, anticipación, penalidades y necesidad de aprobación de cada área común. | 6 | Bejarano Martinez, Alvaro Leandro | Done |
| US04 | Verificar información de usuarios | T51 | Implementar consulta de información de usuarios | Crear una vista administrativa para revisar los datos de propietarios, residentes e inquilinos, incluyendo su información de contacto, documento y unidad asignada. | 3 | Sarmiento Medina, Loreley | Done |
| US07 | Registrar edificio y unidades | T52 | Implementar registro de edificios y unidades | Desarrollar la funcionalidad para registrar edificios, bloques, pisos y unidades residenciales, asociándolos correctamente dentro de la estructura del condominio. | 7 | Becerra Tejeda, Alessandra Nicole | Done |
| US13 | Publicar comunicados oficiales | T53 | Implementar publicación de comunicados | Permitir que el administrador cree y publique comunicados oficiales dirigidos a todos los residentes o a grupos específicos del condominio. | 4 | Ortiz Cardenas, Johanna Antuanete | Done |
| US23 | Registrar pagos en el sistema | T54 | Implementar registro de pagos | Desarrollar el registro de pagos realizados por los residentes, asociándolos a una deuda, unidad, concepto, monto, fecha y estado correspondiente. | 5 | Zegarra Lopez, Renato Sebastian Rubber | Done |
| US24 | Visualizar residentes morosos | T55 | Implementar consulta de morosidad | Crear una vista que muestre a los residentes con deudas vencidas, incluyendo unidad, monto pendiente, fecha de vencimiento y días de atraso. | 5 | Bejarano Martinez, Alvaro Leandro | Done |
| US32 | Consultar leyes y manuales | T56 | Implementar módulo de leyes y manuales | Desarrollar una sección que permita consultar reglamentos internos, manuales de convivencia, políticas del condominio y documentos legales relevantes. | 4 | Sarmiento Medina, Loreley | Done |
| US40 | Ver historial de uso de áreas comunes | T57 | Implementar historial de reservas | Crear una vista administrativa con el historial de uso de las áreas comunes, mostrando residente, unidad, fecha, horario, estado y área reservada. | 5 | Becerra Tejeda, Alessandra Nicole | Done |
